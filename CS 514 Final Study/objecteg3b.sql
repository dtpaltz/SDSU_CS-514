...
-- themselves;  A TABLE OF OBJECTS is said to have an "object column"
-- in objecteg3c.sql we create the table a different way;  note that
-- this is a more purely object oriented approach; an employee is not
-- a scalar type intuitively.  Since we have a 'table of objects',
-- this would be a good map from a java or C++ vector, since in OOP
-- languages, a vector contains objects, and is 'unbounded', just as a
-- database table is 'unbounded' in its number of rows.
...
/
SHOW ERRORS
-- object column below is named
CREATE TABLE emped3 (emp t_emped);
